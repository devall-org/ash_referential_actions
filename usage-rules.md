...
- `do_nothing_*`: normal Ash relationship with no lifecycle behavior added by this extension.
...
Lifecycle reverse relationships (`cascade/restrict/nilify has_many/has_one`) must be unfiltered, attributable, and manual-free. Add separate `do_nothing_has_many/do_nothing_has_one` relationships for filtered business views.
...
do_nothing_belongs_to :private_pointer, File, allow_nil?: true, public?: false
```
...
A target destroy invokes the generated action through the canonical `nilify_has_many/has_one` relationship. Keep that reverse relationship unfiltered; use additional `do_nothing_*` relationships for filtered lists.
...
`many_to_many` is a query relationship and is not assigned a referential action directly. Declare lifecycle actions on the join resource's two `belongs_to` relationships and on the corresponding canonical reverse relationships. Use `do_nothing_has_many` or `many_to_many` for query views.
