defmodule AshOwnership.Verifiers.UsedByConsistency do
  ...
  # A `used_by` that points at a module which never uses anything is
  # therefore not detectable; this is documented as a limitation.
  #
  # For each `uses` edge to destination D:
  #
  #   a. Every `used_by` on D that points at this resource must match one
  #      of this resource's `uses` edges on BOTH key attributes (no lying
  #      or mis-wired reverse declarations).
  #   b. A plain `has_many` on D that traverses a uses foreign key of this
  #      resource must be declared with `used_by` instead.
  #   c. D must declare a matching `used_by` back to this resource, so
  ...
  defp verify_destination(dsl_state, module, destination, uses_rels) do
    ...
    with :ok <- verify_reverse_authenticity(module, destination, uses_rels, reverse_rels),
         ...
         :ok <- verify_has_reverse(module, destination, uses_rels, reverse_rels) do
      verify_guard_channels(dsl_state, module, destination, uses_rels, reverse_rels)
    end
  end

  # The target-live guard queries D through the uses edge; the destroy
  # guard queries this resource through D's used_by. Both channels'
  # effective read actions are checked here, on the using side, keeping
  # compile-time dependencies one-directional.
  defp verify_guard_channels(dsl_state, module, destination, uses_rels, reverse_rels) do
    Enum.reduce_while(uses_rels, :ok, fn uses_rel, :ok ->
      matching = Enum.filter(reverse_rels, &matching_pair?(&1, uses_rel))

      with :ok <- verify_channel(module, uses_rel, destination, destination, uses_rel.name),
           :ok <- verify_reverse_channels(module, matching, dsl_state, uses_rel) do
        {:cont, :ok}
      else
        {:error, error} -> {:halt, {:error, error}}
      end
  ...
  # The destroy guard queries every `used_by` and blocks if any reports a
  ...
  defp verify_reverse_channels(module, reverse_rels, dsl_state, uses_rel) do
    resolved =
    ...
    cond do
      unresolved ->
        {:error, channel_error(module, uses_rel.name, unresolved)}
      ...
      true ->
        {:error,
         channel_error(module, uses_rel.name, """
         Every `used_by` on #{inspect(uses_rel.destination)} matching \
         `uses :#{uses_rel.name}` reads #{inspect(module)} through a read action that \
         ...
         miss a live user. Add one unfiltered channel:
           ...
           used_by :all_#{suggest_name(module)}, #{inspect(module)}
  ...
  # module for the uses edge, this resource's dsl_state for the reverse.
  ...
  defp verify_reverse_authenticity(module, destination, uses_rels, reverse_rels) do
    ...
    |> Enum.find(fn reverse_rel ->
      not Enum.any?(uses_rels, &matching_pair?(reverse_rel, &1))
    end)
    |> case do
      nil ->
        :ok
      ...
      reverse_rel ->
        {:error,
         Spark.Error.DslError.exception(
           ...
           path: [:relationships],
           message: """
           #{inspect(destination)} declares `used_by :#{reverse_rel.name}` pointing at \
           ...
           matching `uses` relationship on that attribute pair.
           """
         )}
    end
  end

  defp verify_has_reverse(module, destination, uses_rels, reverse_rels) do
    uses_rels
    |> Enum.find(fn uses_rel ->
      not Enum.any?(reverse_rels, &matching_pair?(&1, uses_rel))
    end)
    |> case do
      nil ->
        :ok
        ...
        {:error,
         Spark.Error.DslError.exception(
           ...
           user. Add to #{inspect(destination)}:
             ...
             used_by :#{suggest_name(module)}, #{inspect(module)}
           """
         )}
    end
  end

  # A plain `has_many` over a uses foreign key is only a problem when it is
  # the *only* reverse declaration: the guard would then have no channel. Once
  # a matching `used_by` exists, further plain views over the same key are
  # ordinary derived relationships (filtered lists and the like) and are left
  # alone — `used_by` deliberately accepts no `filter`, so they could not
  # be expressed as one anyway.
  defp verify_plain_has_many(module, destination, uses_rels, destination_rels, reverse_rels) do
    unguarded =
      Enum.reject(uses_rels, fn uses_rel ->
        Enum.any?(reverse_rels, &matching_pair?(&1, uses_rel))
      end)
      ...
      rel ->
        {:error,
         Spark.Error.DslError.exception(
           ...
           path: [:relationships],
           message: """
           #{inspect(destination)} declares `has_many :#{rel.name}` traversing the uses \
           foreign key #{inspect(rel.destination_attribute)} of #{inspect(module)}.

           The reverse side of a uses edge must be declared with `used_by`:

             used_by :#{rel.name}, #{inspect(module)}
           """
         )}
    end
  end
